...
- candidate 用于选举出一个新的 leader(可以看[图4](/posts/2022-10-10-raftkv/#figure-3))。
...
Raft 使用心跳机制来触发 leader 选举。当服务器启动时，它们开始是 follower 。只要服务器收到来自 leader 或 candidate 的有效RPC，
它就一直处于 follower 状态。 leader 定期向所有 follower 发送心跳（`AppendEntries RPCs`，不携带日志条目），以保持他们的权威。
如果 follower 在一段被称为 _election timeout_ 的时间内没有收到任何通信，那么它就认为没有可行的 leader ，
并开始选举以选择一个新的 leader。
