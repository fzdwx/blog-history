...
raft 实例被 kill，这个函数也应该return gracefully。
...
每个**日志包含相同顺序的相同命令**，所以每个状态机处理相同的命令序列。因为**状态机是确定的**，所以每个状态机**
会计算出相同的状态**和**相同顺序的输出**。
...
并与其他服务器上的共识模块进行通讯以确保它们的每一条日志最终都相同(相同的请求有相同的顺序)， 即使有一些服务失败了。一旦服务被正确的复制
